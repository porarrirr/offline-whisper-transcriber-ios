...
iPhone内でAI音声文字起こしを行うアプリです。OpenAIのWhisperモデルを搭載し、リアルタイム録音または音声ファイルから高精度な文字起こしを行います。初回にモデルをダウンロードした後は、文字起こし処理はオフラインで完結します。
...
- **iOS SpeechTranscriber**: 設定のモデル一覧から Apple のオンデバイス音声認識（日本語・英語など）を選択可能。言語モデルはシステム経由でダウンロード（アプリ容量に含まれない）

## 必要環境

- **macOS** 14.0 Sonoma 以降
- **Xcode** 16.0 以降（iOS SpeechTranscriber を利用する場合は iOS 26 SDK 推奨）
...
3. 以下の通り入力：
   ...
   - **Organization Identifier**: `com.porarrirr` など
   - **Interface**: `SwiftUI`
   - **Language**: `Swift`
...
5. **Privacy - Speech Recognition Usage Description** (`NSSpeechRecognitionUsageDescription`) が設定されていることを確認（iOS SpeechTranscriber 利用時）
...
| 設定項目 | 値 |
|---------|-----|
| **Bundle Identifier** | `com.porarrirr.offlinewhispertranscriber` |
...
設定画面で「無音部分をスキップ（VAD）」をオンにし、「VADモデルをダウンロード」を実行してください。
VADモデルは whisper.cpp が対応している `ggml-silero-v6.2.0.bin` を使用し、通常のWhisperモデルとは別にアプリのDocumentsへ保存されます。
...
設定画面でVADモデルの準備状態を確認してください。VADはWhisper本体モデルとは別のモデルファイルを必要とするため、未ダウンロードのままでは処理を開始しません。
